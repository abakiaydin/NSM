...
public class Parser extends Configurable {
	...
	public Parser(CrawlConfig config) throws InstantiationException, IllegalAccessException {
		...
		parseContext.set(HtmlMapper.class, TagMapper.class.newInstance());
	}
	...
	public boolean parse(Page page, String contextURL) {
		...
		// handle urls that are for get method
		int urlCount = 0;
		...
		for (String url : urls) {
			...
			webURL.setFormParams("");
			outgoingUrls.add(webURL);
		}
		parseData.setOutgoingUrls(outgoingUrls);
		
		// update urls of form urls
		Set<Form> forms = new HashSet<Form>();
		for (Form frm : contentHandler.getOutgoingForms()) {
			String href = frm.getUrl();
			href = href.trim();
			if (href.length() == 0) {
				continue;
			}
		...
		// forms will be manipulated and converted into webURL later, in WebCrawler
		parseData.setOutgoingForms(forms);
