...
public class PageFetcher extends Configurable {
	...
	public PageFetchResult fetchHeader(WebURL webUrl) {
		...
		try {
			...
			String method = webUrl.getMethod();
			if (method == null || method.equals("get")) {
				get = new HttpGet(toFetchURL);
				get.addHeader("Accept-Encoding", "gzip");
			}
			else if(webUrl.getMethod().equals("post")) {
				get = new HttpPost(toFetchURL);
				get.addHeader("Accept-Encoding", "gzip");
				get.setHeader(HTTP.CONTENT_TYPE, "application/x-www-form-urlencoded");
				((HttpPost)get).setEntity(new StringEntity(webUrl.getFormParams(), HTTP.UTF_8));
			}
			...
			// get cookies
			fetchResult.setCookies(httpClient.getCookieStore().getCookies());
